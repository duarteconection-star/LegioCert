/**
 * LegioCert Pro - Base de Datos IndexedDB v2
 * Soporte para múltiples instalaciones por tratamiento
 */
const DB = (() => {
  ...
  const STORES = {
    ...
    tratamientos:  { keyPath: 'id', autoIncrement: true, indexes: ['clienteId','fecha'] },
    ...
    request.onupgradeneeded = (e) => {
      ...
      Object.entries(STORES).forEach(([name, schema]) => {
        if (!idb.objectStoreNames.contains(name)) {
          ...
          // Migración v1 → v2: eliminar índice instalacionId de tratamientos si existe
          if (name === 'tratamientos' && oldVersion < 2) {
            const store = e.target.transaction.objectStore(name);
            if (store.indexNames.contains('instalacionId')) {
              try { store.deleteIndex('instalacionId'); } catch(err) {}
            }
          }
        }
      });
  ...
  const add = (storeName, data) =>
    new Promise((resolve, reject) => {
      const request = tx(storeName, 'readwrite').add({
  ...
  // Obtener tratamientos por cliente (usando filtro en cliente)
  const getTratamientosByCliente = async (clienteId) => {
    const todos = await getAll('tratamientos');
    return todos.filter(t => t.clienteId === clienteId);
  };

  // Obtener tratamientos que incluyan una instalación concreta
  const getTratamientosByInstalacion = async (instalacionId) => {
    const todos = await getAll('tratamientos');
    return todos.filter(t => {
      // Soporte para array instalacionIds (nuevo) y instalacionId (legado)
      if (Array.isArray(t.instalacionIds)) {
        return t.instalacionIds.includes(instalacionId);
      }
      return t.instalacionId === instalacionId;
    });
  };
  ...
  return {
    init, getAll, getById, add, update, remove, count, search,
    getTratamientosByCliente, getTratamientosByInstalacion,
    exportAll, importAll, getConfig, setConfig, nextCertNumber,
  };
})();
window.DB = DB;
